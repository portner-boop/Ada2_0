...
public class UploadController {
    ...
    @GetMapping("/files") // Changed endpoint to /api/files
    public ResponseEntity<List<String>> getFileUrls(HttpSession session) {
        try {
            int groupId = (int) session.getAttribute("group_id");
            Groups group = groupHandler.getGroups(groupId);
            ...
            for (Resource resource : resources) {
                if (resource.getGroups() != null && resource.getFile() != null) {
                    ...
                    fileUrls.add(minioService.getPreviewFileUrl(name));
                }
            }

            return ResponseEntity.ok(fileUrls);
        } catch (Exception e) {
            System.err.println("Error generating file URLs: " + e.getMessage()); //Log the error for debugging
